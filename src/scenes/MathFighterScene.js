...
Phaser.Scene
{
    ...
    preload(){
        ...
        this.load.image('start-btn', 'images/newstartbtn.webp')
    ...
    create(){
        ...
        let start_button = this.add.image(
            ...
            'start-btn').setInteractive().setScale(0.3)
    ...
    gameStart(){
        ...
        this.player.anims.play('player-standby', true)
        this.enemy.anims.play('enemy-standby', true)
        this.resuultText = this.add.text(
            this.gameHalfWidth,
            200,
            '0',
    ...
    // 5. Get Operator Method
    getOperator(){
        const operator = ['+','-','x',':']
        return operator[Phaser.Math.Between(0,3)]
    }

    // 6. Generate Question Method
    generateQuestion(){
